...
class Client(local):
    ...
    @process
    def get_stats(self, stat_args=None, callback=None):
        ...
        for s in self.servers:
            ...
            if not stat_args:
                ...
                yield s.send_cmd('stats ' + stat_args)
            serverData = {}
    ...
    @process
    def flush_all(self, callback=None):
    ...
    def disconnect_all(self, callback=None):
        for s in self.servers:
            yield s.close_socket()
        if callback is not None:
            callback()

    def delete_multi(self, keys, time=0, key_prefix='', callback=None):
        '''
    ...
    def incr(self, key, delta=1, callback=None):
        ...
        @rtype: int
        """
        self._incrdecr("incr", key, delta, callback)
    ...
    def decr(self, key, delta=1, callback=None):
        ...
        @rtype: int
        """
        self._incrdecr("decr", key, delta, callback)
    ...
    def append(self, key, val, time=0, min_compress_len=0, callback=None):
        ...
        @rtype: int
        '''
        self._set("append", key, val, time, min_compress_len, callback)

    def prepend(self, key, val, time=0, min_compress_len=0, callback=None):
        ...
        @rtype: int
        '''
        self._set("prepend", key, val, time, min_compress_len, callback)
    ...
    def cas(self, key, val, time=0, min_compress_len=0, callback=None):
        ...
        '''
        self._set("cas", key, val, time, min_compress_len, callback=None)
    ...
    @process
    def set_multi(self, mapping, time=0, key_prefix='', min_compress_len=0, callback=None):
        '''
    ...
    @process
    def _set(self, cmd, key, val, time, min_compress_len=0, callback=None):
        self.check_key(key)
        server, key = self._get_server(key)
        if not server:
            callback(0)
        ...
        try:
            ...
            result = yield server.expect("STORED")
            callback(result == "STORED")
            return
        except socket.error, msg:
            if isinstance(msg, tuple): msg = msg[1]
            server.mark_dead(msg)
    ...
    @process
    def _get(self, cmd, key, callback):
        self.check_key(key)
        server, key = self._get_server(key)
        if not server:
            callback(None)
        ...
        try:
            yield server.send_cmd("%s %s" % (cmd, key))
            rkey = flags = rlen = cas_id = None
            if cmd == 'gets':
                rkey, flags, rlen, cas_id, = yield self._expect_cas_value(server)
                if rkey:
    ...
    def gets(self, key, callback):
        ...
        @return: The value or None.
        '''
        self._get('gets', key, callback)

    @process
    def get_multi(self, keys, key_prefix='', callback=None):
        '''
    ...
    @async
    @process
    def _expect_cas_value(self, server, line=None, callback=None):
        if not line:
            line = yield server.readline()

        if line and line[:5] == 'VALUE':
            resp, rkey, flags, len, cas_id = line.split()
            callback(rkey, int(flags), int(len), int(cas_id))
        else:
...
class AsyncClient(Client):
    ...
    @async
    def get_stats(self, stat_args=None, callback=None):
        super(AsyncClient, self).get_stats(stat_args, callback)

    @async
    def get_slabs(self, callback):
        super(AsyncClient, self).get_slabs(callback)

    @async
    def flush_all(self, callback):
        super(AsyncClient, self).flush_all(callback)

    @async
    def disconnect_all(self, callback):
        super(AsyncClient, self).disconnect_all(callback)

    @async
    def delete_multi(self, keys, time=0, key_prefix='', callback=None):
        super(AsyncClient, self).delete_multi(keys, time, key_prefix, callback)

    @async
    def delete(self, key, time=0, callback=None):
        super(AsyncClient, self).delete(key, time, callback)

    @async
    def incr(self, key, delta=1, callback=None):
        super(AsyncClient, self).incr(key, delta, callback)

    @async
    def decr(self, key, delta=1, callback=None):
        super(AsyncClient, self).decr(key, delta, callback)

    @async
    def add(self, key, val, time=0, min_compress_len=0, callback=None):
        super(AsyncClient, self).add(key, val, time, min_compress_len, callback)

    @async
    def append(self, key, val, time=0, min_compress_len=0, callback=None):
        super(AsyncClient, self).append(key, val, time, min_compress_len, callback)

    @async
    def prepend(self, key, val, time=0, min_compress_len=0, callback=None):
        super(AsyncClient, self).prepend(key, val, time, min_compress_len, callback)

    @async
    def replace(self, key, val, time=0, min_compress_len=0, callback=None):
        super(AsyncClient, self).replace(key, val, time, min_compress_len, callback)

    @async
    def set(self, key, val, time=0, min_compress_len=0, callback=None):
        super(AsyncClient, self).set(key, val, time, min_compress_len, callback)

    @async
    def cas(self, key, val, time=0, min_compress_len=0, callback=None):
        super(AsyncClient, self).cas(key, val, time, min_compress_len, callback)

    @async
    def set_multi(self, mapping, time=0, key_prefix='', min_compress_len=0, callback=None):
        super(AsyncClient, self).set_multi(mapping, time, key_prefix, min_compress_len, callback)

    @async
    def get(self, key, callback):
        super(AsyncClient, self).get(key, callback)

    @async
    def gets(self, key, callback):
        super(AsyncClient, self).gets(key, callback)

    @async
    def get_multi(self, keys, key_prefix='', callback=None):
        super(AsyncClient, self).get_multi(keys, key_prefix, callback)

@contextmanager
def client_from_pool(pool, active, create_fn, request_id):
    log.info("event=client_from_pool.start request_id=%s",
             request_id)
    if pool:
        return_to_pool = True
        client = pool.popleft()
        log.debug("event=client_from_pool.get_from_pool request_id=%s pool_size=%d",
                  request_id,
                  len(pool))
    else:
        return_to_pool = False
        client = create_fn()
        log.debug("event=client_from_pool.create_new_client request_id=%s",
                  request_id)

    active.append(client)

    log.debug("event=client_from_pool.activate_client request_id=%s active=%d",
              request_id,
              len(active))
    ...
    if return_to_pool:
        pool.appendleft(client)
        log.debug("event=client_from_pool.return_to_pool request_id=%s pool_size=%d",
                  request_id,
                  len(pool))

    log.info("event=client_from_pool.end request_id=%s status=0",
             request_id)
...
class ClientPool(object):
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            log.info("event=ClientPool.__new__ msg=\"create new pool\"")
            cls._instance = super(ClientPool, cls).__new__(cls)
        return cls._instance

    def __init__(self, create_client_fn, pool_size=5):
        clients = [create_client_fn() for i in range(0, pool_size)]

        self.pool = collections.deque(clients)
        self.active = collections.deque()
        self.create_client_fn = create_client_fn

    @process
    def pooled_request(self, name, request_id, *args, **kwargs):
        callback = kwargs["callback"]
        del kwargs["callback"]

        with client_from_pool(self.pool, self.active,
                              self.create_client_fn, request_id) as client:
    ...
    def __getattr__(self, name):
        return partial(self.pooled_request, name)

class AsyncClientPool(object):
    def __init__(self, request_id, memcached_hosts, cbwrapper=None, pool_size=5):
        log.debug("event=AsyncClientPool.__init__ request_id=%s",
                 request_id)
        self.request_id = request_id
        self.cbwrapper = cbwrapper
        self.pool = ClientPool(partial(AsyncClient, memcached_hosts), pool_size)

    def __getattr__(self, name):
        log.debug("event=AsyncClientPool.__getattr__ request_id=%s method=%s",
                  self.request_id,
                  name)

        method = partial(getattr(self.pool, name), self.request_id)

        if self.cbwrapper is not None:
            async_method = async(method,
                                 cbwrapper=self.cbwrapper)
        else:
            async_method = async(method)
...
class _Host(object):
    ...
    def __init__(self, host, debug=0, io_loop=None):
        self.debug = debug
    ...
    @async
    def close_socket(self, callback=None):
        if self.stream:
            if callback is not None:
    ...
    @async
    def send_cmds(self, cmds, callback):
        """ cmds already has trailing \r\n's applied """
        self.stream.write(cmds, callback)

    @async
    @process
    def readline(self, callback):
        data = yield async(self.stream.read_until)('\r\n')
        callback(data[:-2])
